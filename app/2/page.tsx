...
import { ArrowDown, ArrowRight, Cable, Clock3, Guitar, Headphones, Instagram, MapPin, Menu, MessageCircle, Music2, PackageCheck, Phone, Piano, Play, Settings2, Star, Volume2, Wrench, X, Youtube } from "lucide-react";
import "./page.css";
import "./services.css";
...
const services=[
 {icon:PackageCheck,title:"Instrument Sales",text:"Browse and compare instruments for beginners, students, hobbyists and performers.",items:["Guitars and keyboards","Indian instruments","Drums and percussion"]},
 {icon:Wrench,title:"Repair & Maintenance",text:"Practical repair support to help restore playability and keep your instrument dependable.",items:["General repair enquiries","Hardware and part checks","Maintenance guidance"]},
 {icon:Settings2,title:"Setup & Tuning",text:"Get help with basic setup, tuning and adjustments before or after your purchase.",items:["Guitar setup support","Tuning assistance","Playability adjustments"]},
 {icon:Cable,title:"Accessories",text:"Find everyday essentials and replacement accessories needed for practice and performance.",items:["Strings and cables","Stands and straps","Cases and small essentials"]},
 {icon:Volume2,title:"Audio Equipment",text:"Ask about microphones, speakers and supporting audio gear available through the store.",items:["Microphone enquiries","Speaker options","Basic audio guidance"]},
 {icon:MessageCircle,title:"Stock Enquiries",text:"Check current availability, ask for suitable options and plan your store visit quickly.",items:["WhatsApp availability check","Direct phone assistance","Visit planning"]}
];
...
export default function MusicTwo(){
 ...
 return <main className="m2">
   ...
   <header className="m2-header">
    ...
    <nav className={open?"open":""}><a href="#sounds" onClick={()=>setOpen(false)}>Sounds</a><a href="#services" onClick={()=>setOpen(false)}>Services</a><a href="#repair" onClick={()=>setOpen(false)}>Repair</a><a href="#story" onClick={()=>setOpen(false)}>Story</a><a href="#visit" onClick={()=>setOpen(false)}>Visit</a><a href={whatsapp} target="_blank" className="m2-nav-cta">Enquire <ArrowRight/></a></nav>
   ...
   <section className="m2-hero" id="top">
    ...
    <div className="m2-hero-copy">
      ...
      <div className="m2-actions"><a href="#sounds">Explore the store <ArrowDown/></a><a href={whatsapp} target="_blank" className="m2-whatsapp-inline"><MessageCircle/> WhatsApp</a></div>
    </div>
   ...
   <section className="m2-services" id="services">
     <div className="m2-services-head"><span>02 / SERVICES</span><h2>Everything around<br/>the instrument.</h2></div>
     <div className="m2-services-grid">
       {services.map(({icon:Icon,title,text,items},index)=><article className="m2-service-card" key={title}>
         <div className="m2-service-top"><span className="m2-service-icon"><Icon/></span><span className="m2-service-no">0{index+1}</span></div>
         <h3>{title}</h3><p>{text}</p><ul>{items.map(item=><li key={item}>{item}</li>)}</ul>
       </article>)}
     </div>
     <div className="m2-service-cta"><div><span>NOT SURE WHAT YOU NEED?</span><strong>Describe it directly to the shop.</strong></div><a href={whatsapp} target="_blank" className="m2-whatsapp-inline"><MessageCircle/> Ask on WhatsApp</a></div>
   </section>
   ...
   <section className="m2-repair" id="repair">
    ...
    <div className="m2-repair-copy"><span>03 / KEEP IT PLAYING</span><h2>Your instrument has a life after the purchase.</h2><p>Bring your instrument for an initial check and speak directly with the store about setup, maintenance or possible repair work. Availability and repair scope can be confirmed before your visit.</p><a href="tel:+919319583602">Call repair desk <ArrowRight/></a></div>
   </section>

   <section className="m2-story" id="story">
    <div className="m2-story-top"><span>04 / THE STORE</span><p>Near Sai Dham Mandir · Sector 86</p></div>
